...
def response_time(dff):
    ...
    # Convert 'stim_rating' from string to two columns: 'taste' and 'health'
    ...
    # Drop rows where 'taste' or 'health' are NaN after conversion
    extracted_data = extracted_data.dropna(subset=['taste', 'health'])
    
    # Ensure 'taste' and 'health' are integers
    extracted_data['taste'] = extracted_data['taste'].astype(int)
    extracted_data['health'] = extracted_data['health'].astype(int)

    # Apply random choice for taste and health when the value is 0
    ...
    # Apply filters based on taste and health conditions
